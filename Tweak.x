...
static NSFileManager *g_fileManager = nil; // đối tượng quản lý file
static UIPasteboard *g_pasteboard = nil; // đối tượng clipboard
static BOOL g_canReleaseBuffer = YES; // có thể giải phóng buffer hiện tại không
static BOOL g_bufferReload = YES; // có cần reload file video ngay không
static AVSampleBufferDisplayLayer *g_previewLayer = nil; // preview camera gốc
static NSTimeInterval g_refreshPreviewByVideoDataOutputTime = 0; // nếu có VideoDataOutput, preview đồng bộ theo VideoDataOutput; nếu không thì đọc video trực tiếp
...
+ (CMSampleBufferRef _Nullable)getCurrentFrame:(CMSampleBufferRef _Nullable) originSampleBuffer :(BOOL)forceReNew{
    ...
    // không có video thay thế thì trả về nil để dùng dữ liệu gốc
    if ([g_fileManager fileExistsAtPath:g_tempFile] == NO) return nil;
    if (sampleBuffer != nil && !g_canReleaseBuffer && CMSampleBufferIsValid(sampleBuffer) && forceReNew != YES) return sampleBuffer; // không thể giải phóng buffer thì trả về buffer trước


    static NSTimeInterval renewTime = 0;
    // đã chọn video thay thế mới
    ...
    if (g_bufferReload) {
        ...
        @try{
            ...
            NSLog(@"Lỗi khởi tạo đọc video: %@", except);
        }
    }
    // NSLog(@"刷新了");
    ...
    // copy đúng loại theo subMediaType
    switch(subMediaType) {
    ...
    if (newsampleBuffer == nil) {
        ...
        if (originSampleBuffer != nil) {
            ...
            // nếu có buffer gốc thì tạo buffer mới theo format của nó
            ...
            // đọc trực tiếp từ video dạng kCVPixelFormatType_32BGRA
            sampleBuffer = newsampleBuffer;
        }
    }
...
-(void) startRunning {
	...
	NSLog(@"Bắt đầu dùng camera, preset: %@", [self sessionPreset]);
	%orig;
}
-(void) stopRunning {
    g_cameraRunning = NO;
	NSLog(@"Dừng dùng camera");
	%orig;
}
        ...
        if (@available(iOS 11.0, *)){ // iOS 11 之后
            ...
            MSHookMessageEx(
                ...
                imp_implementationWithBlock(^(id self, AVCapturePhotoOutput *captureOutput, AVCapturePhoto *photo, NSError *error){
                    ...
                    // không có buffer, tạm thời tạo một cái
                    ...
                    if (tempBuffer != nil) CFRelease(tempBuffer); // giải phóng buffer tạm

                    if (newBuffer != nil) { // nếu có dữ liệu thay thế thì hook các property
                        if (copyBuffer != nil) CFRelease(copyBuffer);
                        CMSampleBufferCreateCopy(kCFAllocatorDefault, newBuffer, &copyBuffer);
                        ...
                        CIContext *cicontext = [CIContext new]; // vấn đề xoay ảnh
                        ...
                        // sau khi có buffer mới, bắt đầu hook các property
    ...
    if ([hooked containsObject:className] == NO) {
        ...
        MSHookMessageEx(
            ...
            imp_implementationWithBlock(^(id self, AVCaptureOutput *output, CMSampleBufferRef sampleBuffer, AVCaptureConnection *connection){
                ...
                if (nowTime - oldTime > 3000) { // refresh mỗi 3 giây
                    oldTime = nowTime;
                    // camera info
...
static NSString *g_downloadAddress = @""; // địa chỉ tải video
static BOOL g_downloadRunning = NO; // đang tải xuống không
...
/**
 * tải video xuống
 * @param bool quick có phải tải nhanh không, nếu có thì giảm bớt popup
 */
...
-(void)decreaseVolume {
    ...
    if (g_volume_up_time != 0 && nowtime - g_volume_up_time < 1) {
        ...
        // thông tin độ phân giải trên clipboard
        NSString *str = g_pasteboard.string;
        NSString *infoStr = @"Mở camera để ghi lại thông tin";
        ...
        UIAlertAction *next = [UIAlertAction actionWithTitle:@"Chọn video" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
            ui_selectVideo();
        }];
        UIAlertAction *download = [UIAlertAction actionWithTitle:@"Tải video" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
            UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"Tải video" message:@"Nên dùng định dạng MOV\nMP4 cũng được, loại khác chưa thử nghiệm" preferredStyle:UIAlertControllerStyleAlert];
            [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
                if ([g_downloadAddress isEqual:@""]) {
                    textField.placeholder = @"Địa chỉ video từ xa";
                }else {
                    textField.text = g_downloadAddress;
                }
                textField.keyboardType = UIKeyboardTypeURL;
            }];
            UIAlertAction* okAction = [UIAlertAction actionWithTitle:@"Xác nhận" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
                g_downloadAddress = alert.textFields[0].text;
                NSString *resultStr = @"Chế độ nhanh đã đổi sang tải từ xa\n\nCần đảm bảo link video có thể truy cập\n\nSau khi xong sẽ có thông báo tắt tiếng\nTải thất bại sẽ tắt thay thế";
                if ([g_downloadAddress isEqual:@""]) {
                    resultStr = @"Chế độ nhanh đã đổi sang chọn từ thư viện";
                }
                UIAlertController* resultAlert = [UIAlertController alertControllerWithTitle:@"Thay đổi chế độ nhanh" message:resultStr preferredStyle:UIAlertControllerStyleAlert];

                UIAlertAction *ok = [UIAlertAction actionWithTitle:@"Đã hiểu" style:UIAlertActionStyleDefault handler:nil];
            ...
            UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"Huỷ" style:UIAlertActionStyleDefault handler:nil];
            [alert addAction:okAction];
            [alert addAction:cancel];
            [[GetFrame getKeyWindow].rootViewController presentViewController:alert animated:YES completion:nil];
        }];
        UIAlertAction *cancelReplace = [UIAlertAction actionWithTitle:@"Tắt thay thế" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action){
            if ([g_fileManager fileExistsAtPath:g_tempFile]) [g_fileManager removeItemAtPath:g_tempFile error:nil];
        }];

        NSString *isMirroredText = @"Sửa lỗi ảnh bị lật";
        if ([g_fileManager fileExistsAtPath:g_isMirroredMark]) isMirroredText = @"Sửa lỗi ảnh bị lật ✅";
        ...
        UIAlertAction *showHelp = [UIAlertAction actionWithTitle:@"- Xem hướng dẫn -" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
            NSURL *URL = [NSURL URLWithString:@"https://github.com/codebywin/vcamios-rootless/blob/main/index.html"];
...
%ctor {
    ...
    g_isMirroredMark = [NSString stringWithFormat:@"%@/var/mobile/Library/Caches/vcam_is_mirrored_mark", jbroot];
    g_tempFile = [NSString stringWithFormat:@"%@/var/mobile/Library/Caches/temp.mov", jbroot];
